...
export function registerSRLLanguage(monaco: Monaco) {
  ...
  monaco.languages.setLanguageConfiguration(SRL_LANGUAGE_ID, srlLanguageConfiguration);
  monaco.languages.setMonarchTokensProvider(SRL_LANGUAGE_ID, srlMonarchTokensProvider);
  ...
  monaco.languages.registerHoverProvider(SRL_LANGUAGE_ID, {
    provideHover: (model, position) => {
      ...
      if (textBefore.endsWith('?') || textBefore.endsWith('$')) {
        ...
        return {
          ...
          contents: [
            ...
            { value: 'A query variable that binds to RDF terms during rule evaluation.' },
          ],
        };
      }
      
      // Prefixed name hover
      const prefixedNameMatch = fullText.match(/^([a-zA-Z_][a-zA-Z0-9_-]*):([a-zA-Z_][a-zA-Z0-9_-]*)/);
      ...
      const keywordDocs: Record<string, { title: string; description: string; syntax?: string }> = {
        'RULE': {
          title: 'Rule Declaration',
          description: 'Declares a rule that derives new triples from matched patterns.',
        ...
        'IF': {
          title: 'If Condition',
          description: 'Alternative rule syntax - if the body matches, derive the head.',
        ...
        'NOT': {
          title: 'Negation',
          description: 'Negation-as-failure: matches if the pattern does NOT exist.',
          syntax: 'NOT { pattern }',
        },
        'TRANSITIVE': {
          title: 'Transitive Property',
          description: 'Declares a property as transitive (if A→B and B→C, then A→C).',
